...
int output_open_cl_file(
  ...
  if (pop->write_header == _TRUE_) {
    ...
    if (pop->output_format == class_format) {
      ...
      class_fprintf_columntitle(*clfile,"TB",psp->has_tb,colnum);
      class_fprintf_columntitle(*clfile,"EB",psp->has_eb,colnum);
      /* Ends here */
    }
    else if (pop->output_format == camb_format) {
      class_fprintf_columntitle(*clfile,"TT",psp->has_tt,colnum);
...
int output_one_line_of_cl(
  ...
  if (pop->output_format == camb_format) {
    ...
    /* Modification starts here */
    class_fprintf_double(clfile, factor*pow(pba->T_cmb*1.e6,2)*cl[psp->index_ct_tb], psp->has_tb);
    class_fprintf_double(clfile, factor*pow(pba->T_cmb*1.e6,2)*cl[psp->index_ct_eb], psp->has_eb);
    /* Ends here */
    index_ct_rest = 0;
    ...
    if (psp->has_ep == _TRUE_)
      index_ct_rest++;
    if (psp->has_tb == _TRUE_)
      index_ct_rest++;
    if (psp->has_eb == _TRUE_)
      index_ct_rest++;
